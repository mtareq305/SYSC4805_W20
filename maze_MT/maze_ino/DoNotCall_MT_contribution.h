/*
 * MT_contribution.h created for Purpose of individual code review on MARCH 30, 2020
 *	IT is not to be use for any main function.
...
#include <Arduino.h>
#include <ArduinoUnit.h> //Install ArduinoUnit test framework library.
#include <avr/wdt.h> // import from Arduino.h and AVR atmega328p system framework library.
#include "maze_header.h" // Project code header file, includes Robots functional code.
#include <Battery.h> // Install BatterySense library


#ifndef MAZE_INO_MT_CONTRIBUTION_H_
#define MAZE_INO_MT_CONTRIBUTION_H_





#endif /* MAZE_INO_MT_CONTRIBUTION_H_ */
...
/*
This version is created by Muhammad Tarequzzaman But not being implemented by the group
*/
...
void State(uint8_t leftSensorValue,uint8_t rightSensorValue,uint8_t SideRightSensorValue,uint8_t SideLeftSensorValue){
	// select the state
		...
		case 3: // 0 : 0
			...
			stop(); //Stop
			left360(); // left turn
			if (pathArray[index-1] != "TurnAround") {
...
/*This setup for Battery sense, unit TEST and WDT*/
